...
library(RSQLite)
#Conexao com SQL
con <-dbConnect(SQLite(), "C:\\Users\\jvsca\\Desktop\\Unicamp\\Banco de Dados\\lista 5\\me315 (2).sqlite")
...
```{sql connection=con}
SELECT PNAME,
       ACRES AS CURRENTAREA,
       ACRES * 1.2 AS NEWAREA
FROM PRESERVE
...
```{sql connection=con}
SELECT COUNT(DISTINCT FEE) AS DISTINCTFEES
FROM PRESERVE
WHERE FEE <> 0
...
```{sql connection=con}
SELECT AVG(ACRES * 0.02) AS NEWAVGFEE
FROM PRESERVE
WHERE STATE = 'AZ'
...
```{sql connection=con}
SELECT COUNT(DISTINCT DNO) AS DEPTCOUNT
FROM EMPLOYEE
